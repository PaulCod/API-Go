...
// Usuarios reqpresenta um repositorio de ususarios
...
// Deletar exclui as informações de usuario de um banco de dados
...
// BuscarSeguindo traz todos os usuarios que um determinado usuario esta seguindo
func (repositorio Usuarios) BuscarSeguindo(usuarioID uint64) ([]modelos.Usuario, error) {
	linhas, erro := repositorio.db.Query(`
		select u.id, u.nome, u.nick, u.email, u.criadoEm 
		from usuarios u inner join seguidores s on u.id = s.usuario_id where seguidor_id = ?`, usuarioID)
	if erro != nil {
		return nil, erro
	}
	defer linhas.Close()
...
func (repositorio Usuarios) AtualizarSenha(usuarioID uint64, senha string) error {
	instantment, erro := repositorio.db.Prepare("update usuarios set senha = ? where id = ?")
	if erro != nil {
		return erro
	}
	defer instantment.Close()

	if _, erro := instantment.Exec(senha, usuarioID); erro != nil {
		return erro
	}
