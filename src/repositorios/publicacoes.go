...
// Criar insere uma publicção no banco de dados
...
// Buscar traz as publicaçoes dos usuarios seguidos e tambem do proprio usuario que fez a requisição
func (repositorio Publicacoes) Buscar(usuarioID uint64) ([]modelos.Publicacao, error) {
	linhas, erro := repositorio.db.Query(`
		select distinct p.*, u.nick from publicacoes p
		inner join usuarios u on u.id = p.autor_id
		inner join seguidores s on p.autor_id = s.usuario_id 
		where u.id = ? or seguidor_id = ? order by 1 desc
	`, usuarioID, usuarioID)
	if erro != nil {
		return nil, erro
	}
...
// BuscarPorUsuario traz todas as oublicações de um usuario especifico
